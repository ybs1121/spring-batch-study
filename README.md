...
# Spring Batch 생존 전략 요약

## 1. JdbcCursorItemReader (커서 기반 처리)
- 데이터베이스와 커넥션을 유지하며 데이터를 순차적으로 가져옵니다.
- 하나의 커넥션으로 데이터를 스트리밍하듯 처리하여 메모리 사용을 최소화합니다.
- 데이터는 ResultSet에서 커서를 이동하며 한 행씩 읽습니다.
- 장점: 메모리 사용량 최소, 대용량 데이터에도 효과적.
- 단점: 긴 배치 작업 시 DB 커넥션이 오래 유지되어 리소스 부담 발생 가능.

## 2. JdbcPagingItemReader (페이징 기반 처리)
- 데이터를 페이지 단위로 쪼개어 처리합니다.
- 각 페이지마다 새로운 쿼리를 실행해 커넥션을 짧게 유지하며 안정성을 높입니다.
- 메모리에 한 페이지 분량의 데이터만 로드하므로 메모리 부담이 적습니다.
- 멀티스레드 환경에서 안전하며, 페이징 시 정렬(order by) 구문이 필수입니다.
